...
| **CI-2** | Tag `v1.0.0` y primera ejecución de `production.yml` | 15 min | **hecho** — [`v1.0.0`](https://github.com/Gameoversv/Sistema-de-Gestion-de-Inventarios-Empresarial/releases/tag/v1.0.0) publicado el 2026-07-28 desde el [run 30347306408](https://github.com/Gameoversv/Sistema-de-Gestion-de-Inventarios-Empresarial/actions/runs/30347306408): tests, imagen de producción y Release, los tres en verde. **Destapó un 403 latente**: el repositorio tiene `default_workflow_permissions: read` y el job no declaraba permisos, así que `Create GitHub Release` habría muerto con 403 *después* de correr los tests y construir la imagen. Añadido `permissions: contents: write` (PR #89), el mismo patrón que ya usaba `promote-to-staging.yml`. El workflow llevaba escrito desde el principio sin haberse ejecutado nunca: etiquetar fue lo que lo probó |
| **—** | Smoke test post-release | 45 min | pendiente — ya desbloqueado, CI-2 cerrado |
...
|---|---|---|---|
| **`docs/requisitos/`** | "documento detallado de requisitos funcionales y no funcionales" | 3 h | **hecho** — **22 RF y 24 RNF**, cada uno con su cita del PDF, el `fichero:línea` que lo implementa y la prueba que lo verifica. **44 cumplen, 2 parciales y cero pendientes** tras el barrido: RNF-05 lo cerró G-1, RNF-08 resultó ser documentación desfasada y RNF-21 lo cerró CI-2 al ejecutar `production.yml` con `v1.0.0`. Los dos parciales son límites conocidos —Jenkins por C-4 y las revisiones cruzadas—, no trabajo olvidado |
...
| ~~**A-2 / M-1**~~ | ~~`UserController` sobre la Admin API~~ — **hecho**: `/api/users` con listar, crear, editar, activar/desactivar, reemplazar roles y eliminar, todo tras `SCOPE_user:manage`; más la sección Usuarios del frontend. El backend habla con la Admin API como service account de `inventory-backend`, capacidad que ya existía desde G-1. Cierra #48 y completa la matriz de 7 permisos | 4 h | — |
...
| Smoke test post-release — CI-2 ya cerrado con `v1.0.0` | 0,25 | 45 min | 0,33 |
| Ola 8 restante (A-2/M-1, A-1) por ADR | 0,15 | 1 h | 0,15 |

**Cerradas:** Olas 2, 3, 5 y 7 completas, la Ola 4 salvo C-4 (CI-2 cerrado con `v1.0.0`), y **todas las mejoras funcionales** (D-1, D-2, D-3, D-4, E-2, F-1, F-2, M-2, S-2, SEC-2).
