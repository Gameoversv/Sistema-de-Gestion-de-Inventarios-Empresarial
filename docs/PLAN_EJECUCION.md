...
| **G-8** | `scope-mappings` por rol en `init-users.sh`, atando cada client scope a los roles autorizados. **Implementado, pendiente del veredicto de CI**: el paso "G-8" de `staging.yml` decodifica el token de un viewer que pide scopes elevados y comprueba que Keycloak no los emite. Sospecha técnica: los scope-mappings gatean los *roles* del token, no la *cadena* `scope`; si el test sale rojo, G-8 necesita otro enfoque (mappers condicionales o quitar los scopes de *optional*). No se verifica en local por C-4. El mapa Java (ADR-002) no se retira hasta que el test esté verde | 1 h |
